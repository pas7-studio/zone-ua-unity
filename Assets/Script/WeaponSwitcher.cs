...
public sealed class WeaponSwitcher : MonoBehaviour
{
    ...
    public int ActiveWeaponIndex => Array.IndexOf(weapons, selectedWeapon);
    ...
    {
        if (isSwitching || index < 0 || index >= weapons.Length || weapons[index] == null) return;
        if (selectedWeapon == weapons[index]) return;
        if (switchRoutine != null) StopCoroutine(switchRoutine);
        switchRoutine = StartCoroutine(SwitchWeaponRoutine(weapons[index], animators[index], controllers[index]));
    }

    private IEnumerator SwitchWeaponRoutine(GameObject newWeapon, Animator newAnimator, WeaponController newController)
    {
        isSwitching = true;
        ...
        if (switchingTime > 0f) yield return new WaitForSeconds(switchingTime);
        newController?.WeaponChanged();
        if (newAnimator != null) newAnimator.enabled = false;
        isSwitching = false;
        switchRoutine = null;
    }
    ...
    {
        if (selectedWeapon == weaponObject && ActiveWeaponController == controller) return;
    ...
    {
        for (int i = 0; i < weapons.Length; i++)
            if (weapons[i] != null) weapons[i].SetActive(state);
    }

    private void OnDisable() => ActiveWeaponController?.StopFire();

    private void OnValidate() => switchingTime = Mathf.Max(0f, switchingTime);
}
